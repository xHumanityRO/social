...
public interface InstagramInsightRepository  extends JpaRepository<InstagramInsight, Integer> {
	...
	@Query(value = "select DATE(a.creation_time) as day, sum(a.likes - a.dislikes + (\n" + 
			...
			"	where a.id in (select max(ii.id) from instagram_insight ii group by media_url, DATE(ii.creation_time))\n" + 
			"	group by DATE(a.creation_time)\n" + 
			"	order by DATE(a.creation_time)", nativeQuery = true)
